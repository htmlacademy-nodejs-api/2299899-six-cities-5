...
import { STATIC_FILES_ROUTE, STATIC_UPLOAD_ROUTE } from '../../../../rest/rest.const.js';
      ...
      for (const key in current) {
        if (Object.hasOwn(current, key)) {
          ...
          if (this.isStaticProperty(key) && typeof value === 'string') {
            ...
            const roorPath = this.hasDefaultImage(value) ? staticPath : uploadPath;
            current[key] = `${getFullServerPath(serverHost, serverPort)}${roorPath}/${value}`;
          }
        }
      }
    }
